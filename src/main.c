/**
 * ============================================================================
 * FAN SPEED CONTROL + DS18B20 TEMPERATURE MONITOR
...
#define TEMP_HOT            40.0f       // Di atas ini = PANAS (LED Merah)
                                        // Antara keduanya = HANGAT (LED Kuning)
...
int main(void)
{
    ...
    /* Test LED saat startup - nyalakan semua 500ms */
    ...
    if (!sensor_ok)
    {
        /* Sensor tidak terdeteksi - LED Merah blink cepat */
        while (1)
        {
...
void DS18B20_SetPinInput(void)
{
    ...
    GPIO_InitStruct.Pull = GPIO_NOPULL;             // External pull-up 4.7K
    GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_HIGH;
    HAL_GPIO_Init(DS18B20_PORT, &GPIO_InitStruct);
}
...
uint8_t DS18B20_Reset(void)
{
    ...
    DS18B20_SetPinInput();
    delay_us(70);                       // Wait 70us untuk presence
    
    if (HAL_GPIO_ReadPin(DS18B20_PORT, DS18B20_PIN) == GPIO_PIN_RESET)
    {
        presence = 1;                   // Sensor detected!
    }
    
    delay_us(410);                      // Wait sisa waktu
    return presence;
}
...
void DS18B20_WriteBit(uint8_t bit)
{
    ...
    if (bit)
    {
        delay_us(1);                    // Write 1: LOW 1us
        DS18B20_SetPinInput();
        delay_us(60);
    }
    else
    {
        delay_us(60);                   // Write 0: LOW 60us
        DS18B20_SetPinInput();
        delay_us(1);
    }
}
...
float DS18B20_ReadTemperature(void)
{
    ...
    temperature = (float)temp_raw / 16.0f;  // Resolusi 12-bit = 0.0625°C
